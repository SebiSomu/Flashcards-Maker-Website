...
const TreeNode = ({ folder, allFolders, level, selectedFolderId, onSelect, onDelete, onCreateSubfolder, onEdit }: TreeNodeProps) => {
    ...
    return (
        <div className="flex flex-col select-none">
            <div className={`group flex items-center hover:bg-base-200/50 rounded-lg pr-2 transition-colors ${selectedFolderId === folder.ID ? "bg-primary/10 text-primary" : ""}`} style={{ paddingLeft: `${level * 12}px` }}>
                ...
                {isEditing ? (
                    <input
                        ...
                        className="input input-bordered input-xs flex-1 h-7 min-h-0 text-xs"
                        autoFocus
                    />
                ) : (
                ...
                <div className="flex opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); setEditName(folder.name); }} className="btn btn-ghost btn-xs text-base-content/50 hover:text-info font-bold">✎</button>
            ...
            {isExpanded && <div className="flex flex-col">{children.map(child => <TreeNode key={child.ID} folder={child} allFolders={allFolders} level={level + 1} selectedFolderId={selectedFolderId} onSelect={onSelect} onDelete={onDelete} onCreateSubfolder={onCreateSubfolder} onEdit={onEdit} />)}</div>}
        </div>
    );
};

const FolderList = ({ folders, onSelectFolder, onCreateFolder, onDeleteFolder, onEditFolder, selectedFolderId }: FolderListProps) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-0.5 max-h-60 overflow-y-auto pr-2">
                ...
                {rootFolders.map(folder => <TreeNode key={folder.ID} folder={folder} allFolders={folders} level={0} selectedFolderId={selectedFolderId} onSelect={onSelectFolder} onDelete={setFolderToDelete} onCreateSubfolder={handleStartCreate} onEdit={onEditFolder} />)}
            </div>
            {isCreating && (
